...
![Conceptual Arduino Pro Mini board pin reference](../../assets/illustrations/boards/arduino-pro-mini.svg)

> **Role:** small low-power embedded deployment. Typical Indian retail range: **₹250–600** (indicative on 17 July 2026, not a live quote).

| Property | Reference |
|---|---|
| Controller | ATmega328P, 3.3 V/8 MHz or 5 V/16 MHz |
| I/O summary | 14 digital I/O (6 PWM), 6 analog inputs |
| Logic level | Check the board documentation; many pins are 3.3 V-only |
| Alternative | Nano / ATtiny |
...
| Pin / connector | Use |
|---|---|
| `FTDI` | external programming UART |
| `VCC` | regulated rail |
...
The board executes firmware stored in its controller and uses digital/analog peripherals to sample sensors and drive outputs. Choose it for **small low-power embedded deployment**: its processor, voltage domain, memory, connectivity and physical size determine whether it fits. Typical applications include data loggers, control panels, robotics and connected sensor nodes.
...
|---|---|---|
| **Arduino Pro Mini** | small low-power embedded deployment | Check its exact variant, USB interface and voltage limits |
| **Nano / ATtiny** | requirements differ in wireless capability, speed, I/O or power | requires a different toolchain or wiring plan |
...
- Official documentation: [docs.arduino.cc](https://docs.arduino.cc/retired/boards/arduino-pro-mini/)
