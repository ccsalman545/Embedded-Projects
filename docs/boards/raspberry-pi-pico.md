...
![Conceptual Raspberry Pi Pico board pin reference](../../assets/illustrations/boards/raspberry-pi-pico.svg)

> **Role:** fast deterministic I/O and MicroPython. Typical Indian retail range: **₹450–900** (indicative on 17 July 2026, not a live quote).

| Property | Reference |
|---|---|
| Controller | RP2040 dual-core, 3.3 V |
| I/O summary | 26 GPIO; ADC, UART, I²C, SPI, PWM, PIO |
| Logic level | Check the board documentation; many pins are 3.3 V-only |
| Alternative | ESP32 / Arduino Nano |
...
The board executes firmware stored in its controller and uses digital/analog peripherals to sample sensors and drive outputs. Choose it for **fast deterministic I/O and MicroPython**: its processor, voltage domain, memory, connectivity and physical size determine whether it fits. Typical applications include data loggers, control panels, robotics and connected sensor nodes.
...
|---|---|---|
| **Raspberry Pi Pico** | fast deterministic I/O and MicroPython | Check its exact variant, USB interface and voltage limits |
| **ESP32 / Arduino Nano** | requirements differ in wireless capability, speed, I/O or power | requires a different toolchain or wiring plan |
